...
class FriendController extends Controller
{
    ...
    public function request(Request $request): JsonResponse
    {
        ...
        if ($target->id === $user->id) return response()->json(['message' => 'Vous ne pouvez pas vous ajouter vous-m?me.'], 422);
        ...
        if ($existing) {
            if ($existing->status === Friendship::STATUS_ACCEPTED) return response()->json(['message' => 'Vous ?tes d?j? amis.'], 422);
            if ($existing->status === Friendship::STATUS_PENDING) {
                if ($existing->requester_id === $user->id) return response()->json(['message' => 'Demande d?j? envoy?e.'], 422);
                return response()->json(['message' => 'Cet utilisateur vous a d?j? envoy? une demande.'], 422);
            }
            ...
            return response()->json(['message' => 'Demande d'amiti? envoy?e.']);
        }

        Friendship::create(['requester_id' => $user->id, 'addressee_id' => $target->id, 'status' => Friendship::STATUS_PENDING]);
        return response()->json(['message' => 'Demande d'amiti? envoy?e.'], 201);
    }

    public function accept(Request $request, Friendship $friendship): JsonResponse
    {
        $user = $request->user();
        if ($friendship->addressee_id !== $user->id) return response()->json(['message' => 'Action non autoris?e.'], 403);
        if ($friendship->status !== Friendship::STATUS_PENDING) return response()->json(['message' => 'Demande d?j? trait?e.'], 422);
        $friendship->update(['status' => Friendship::STATUS_ACCEPTED]);
        return response()->json(['message' => 'Ami ajout?.']);
    }
    ...
    public function decline(Request $request, Friendship $friendship): JsonResponse
    {
        ...
        return response()->json(['message' => 'Demande refus?e.']);
    }
    ...
    public function remove(Request $request, Friendship $friendship): JsonResponse
    {
        ...
        if (!$isParticipant) return response()->json(['message' => 'Action non autoris?e.'], 403);
        if ($friendship->status === Friendship::STATUS_PENDING && $friendship->requester_id !== $user->id) return response()->json(['message' => 'Action non autoris?e.'], 403);
        $friendship->delete();
        return response()->json(['message' => 'Relation supprim?e.']);
    }
